// src/hooks/useInvoiceSSE.js — Consolidated v12.1
// ---------------------------------------------------------------------
//  Replaces legacy version.
//  Supports: Token Auth, Active Flag, OnEvent Callback
// ---------------------------------------------------------------------
...
export default function useInvoiceSSE(setInvoices, setSelected, active = true, onEvent = null) {
  ...
  useEffect(() => {
    ...
    console.log("🏁 useInvoiceSSE INIT (v12.1 hooks)");
    ...
    es.onerror = (err) => {
      console.log("🔴 SSE ERROR (invoices) - Closing connection:", err);
      es.close(); // 🔥 Prevent infinite retry loop
    };
